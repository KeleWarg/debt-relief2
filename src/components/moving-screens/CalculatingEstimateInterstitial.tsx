...
// Get contractor count estimate based on state (simulated)
function getContractorCount(state: string | null): number {
  if (!state) return 12
  const baseCounts: Record<string, number> = {
    'CA': 47, 'TX': 38, 'FL': 34, 'NY': 31, 'PA': 24,
    'IL': 22, 'OH': 19, 'GA': 18, 'NC': 17, 'MI': 16,
  }
  return baseCounts[state] || Math.floor(Math.random() * 10) + 12
}

interface EstimateAndEmailScreenProps {
 ...
 * 
 * Email capture first, then "Here's what you told us" recap with state/contractor info
 */
export function EstimateAndEmailScreen({ 
  homeSize,
  zipFrom,
  zipTo,
  initialEmail = '',
  onBack,
  onSubmit 
}: EstimateAndEmailScreenProps) {
  const [email, setEmail] = React.useState(initialEmail)
  const [error, setError] = React.useState<string | null>(null)
  
  // Get price range and label for selected size
  ...
  const stateFrom = getStateFromZip(zipFrom)
  const stateTo = getStateFromZip(zipTo)
  const contractorCount = getContractorCount(stateTo)
  
  const isValidEmail = MOVING_VALIDATION.email.pattern.test(email)
...
export { EstimateAndEmailScreen as CalculatingEstimateInterstitial }
export default EstimateAndEmailScreen
