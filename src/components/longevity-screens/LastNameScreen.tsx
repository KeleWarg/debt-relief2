 ...
 * LastNameScreen
 *
 * Screen 11 - Collects user's last name for personalized longevity plan.
 * Superheader "GET YOUR PERSONALIZED PLAN", headline "What is your Last Name?"
 */
export function LastNameScreen({
  initialValue = '',
  onBack,
  onSubmit,
}: LastNameScreenProps) {
  const [lastName, setLastName] = React.useState(initialValue)

  const isValid = lastName.trim().length >= LONGEVITY_VALIDATION.name.minLength

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (isValid) {
      onSubmit?.(lastName.trim())
    }
  }
